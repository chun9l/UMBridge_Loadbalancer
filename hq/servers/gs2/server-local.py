...
class GS2Model(umbridge.Model):
    ...
    def __call__(self, parameters, config):
        ...
        os.system(f"cd ~/gs2dock; mkdir {dt} && cp {input_file} ~/gs2dock/gs2-batch.sh {dt}") # Copy input file and SLURM script into the run folder
        ...
        os.system(f"mv ../{dt} /nobackup/mghw54/gs2runs3/")
        
        # Read results using Pyrokinetics package and print output
        os.chdir(f"/nobackup/mghw54/gs2runs3/{dt}")
